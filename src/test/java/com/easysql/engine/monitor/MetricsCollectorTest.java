...
public class MetricsCollectorTest {
    ...
    public void testRecordSuccessMetrics() {
        ...
        // 记录成功指标
        QueryMetrics success = QueryMetrics.success("test_template", "1.0.0", "test_db", 50, 100, 5);
    ...
    public void testRecordFailureMetrics() {
        ...
        // 记录失败指标
        QueryMetrics failure = QueryMetrics.failure("test_template", "1.0.0", "test_db", 25, "SQL_ERROR", "Invalid syntax");
    ...
    public void testMixedMetrics() {
        ...
        // 记录多个成功和失败
        collector.record(QueryMetrics.success("t1", "1.0.0", "db1", 10, 20, 3));
        collector.record(QueryMetrics.success("t2", "1.0.1", "db1", 15, 25, 7));
        collector.record(QueryMetrics.failure("t3", null, "db2", 5, "TIMEOUT", "Query timeout"));
        collector.record(QueryMetrics.success("t4", "1.0.0", "db2", 12, 18, 2));
    ...
    public void testQueryMetricsFactory() {
        // 测试成功指标构造
        QueryMetrics success = QueryMetrics.success("template1", null, "datasource1", 100, 200, 10);
        ...
        // 测试失败指标构造
        QueryMetrics failure = QueryMetrics.failure("test_template", "1.0.0", "test_db", 25, "SQL_ERROR", "Invalid syntax");
        assertEquals("test_template", failure.templateId);
        assertEquals("test_db", failure.datasource);
        assertEquals(25, failure.buildTimeMs);
        assertEquals(0, failure.executeTimeMs);
        assertEquals(0, failure.rowCount);
        assertFalse(failure.success);
        assertEquals("SQL_ERROR", failure.errorCode);
        assertEquals("Invalid syntax", failure.errorMessage);
    }
}